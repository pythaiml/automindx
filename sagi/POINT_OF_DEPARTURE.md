...
Exactly three, each real `activate(host)` code over the agnostic host surface (`interface.md:11-12`), each closing a gap the audit found (persona not layered; manifest desynced from disk; prose-only modules, no kernel). Together: **be thyself, do no harm, grow thyself.**
